...
diagram is PARSED out of source/Project6Dsp.h, Project6Slots.h,
Project6Sample.h, Project6Stretch.h and Project6Midi.h and evaluated in the
order it is declared, so a diagram that disagrees with the
...
def main():
    ...
    values = read_constants('source/Project6Slots.h', 'source/Project6Sample.h',
                            'source/Project6Stretch.h', 'source/Project6Midi.h',
                            'source/Project6Dsp.h')
    (columns, rows, count,
    ...
    text(draw, (48, 74),
         ...
         'source/Project6Dsp.h, Project6Slots.h, Project6Sample.h, '
         'Project6Stretch.h and Project6Midi.h — re-run it when the routing changes.',
         f_small, fill=MUTED)
    text(draw, (48, 92),
    ...
    # --- geometry ---------------------------------------------------------
    pad_x0, pad_x1 = 62, 196
    fit_x0, fit_x1 = 230, 344
    lvl_x0, lvl_x1 = 380, 508
    bus_x = 558
    rowlvl_x0, rowlvl_x1 = 614, 770
    collect_x = 826
    mix_x = 906
    trim_x0, trim_x1 = 972, 1152
    out_x0, out_x1 = 1220, 1384
    ...
    for i in range(columns):
        ...
        box(draw, (pad_x0, y0, pad_x1, y1), PAD_FILL, (120, 140, 170),
            'Pad A%d' % (i + 1), f_body)
        arrow(draw, pad_x1, mid, fit_x0 - 2, mid)

        # THE TEMPO FIT, first of all - before the pad's own level,
        # because it changes WHEN samples are read and not how loud they
        # are. Off, varispeed or pitch-preserved, per pad.
        box(draw, (fit_x0, y0, fit_x1, y1), (238, 232, 244), (140, 120, 170),
            ('off', 'spd', 'pch')[i % fit_modes], f_body)
        arrow(draw, fit_x1, mid, lvl_x0 - 2, mid)
    ...
    text(draw, ((lvl_x0 + lvl_x1) / 2, bus_bottom + 24), 'one level per pad', f_small,
         fill=MUTED, anchor='mm')
    text(draw, ((fit_x0 + fit_x1) / 2, bus_bottom + 24), 'one fit mode per pad',
    ...
    for r in range(1, rows):
        ...
        text(draw, (pad_x0 + 14, mid),
             'Row %s   —   %d pads  →  %d tempo fits  →  %d slot levels  →  Σ  →  '
             'direct out, then Row %s level'
             % (letter, columns, columns, columns, letter), f_body, anchor='lm')
    ...
    lines = [
        ...
        ('File tempo',   'From the file’s “acid” chunk, else inferred from its '
                         'length assuming a whole number of beats between %.0f and '
                         '%.0f BPM. Neither, and the file plays untouched.'
                         % (bpm_min, bpm_max)),
    ]
    ...
    # --- the MIDI path, which is a different road to the same rows ----
    midi_y = notes_y + len(lines) * 25 + 34
    draw.line((48, midi_y - 18, W - 48, midi_y - 18), fill=RULE, width=1)

    text(draw, (48, midi_y + 4), 'A .mid PAD TAKES THE OTHER ROAD', f_head)
    text(draw, (48, midi_y + 26),
         'A slot holds either kind of file. An audio pad plays into its row’s AUDIO '
         'bus, above; a MIDI pad plays into its row’s EVENT bus, below. Nothing else '
         'about a pad differs — same launch quantise, same bar lines, same drag and drop.',
         f_small, fill=MUTED)

    my = midi_y + 62
    mh = 44

    box(draw, (pad_x0, my, pad_x1, my + mh), (232, 228, 244), (140, 120, 170),
        'MIDI pad C3', f_body)
    arrow(draw, pad_x1, my + mh / 2, fit_x0 - 2, my + mh / 2)

    box(draw, (fit_x0, my, fit_x1, my + mh), BOX_FILL, (170, 170, 175),
    ...
    box(draw, (rowlvl_x0, my, rowlvl_x1, my + mh), (222, 236, 226), BUS_EDGE,
        'channel 3', f_body, sub='= its row', subfnt=f_tiny)
    ...
    midi_notes = [
        ('Loop length', 'The file, run out to the end of the BAR — the project’s bar, '
                        'read every block, so a time-signature change changes the loop.'),
        ('Timing',      'Notes are held in QUARTER NOTES, never seconds, so a MIDI pad is '
